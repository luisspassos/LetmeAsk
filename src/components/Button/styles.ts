...
export const StylizedButton = styled.button<StylizedButtonProps>`
  ...
  background-color: ${({ isOutlined, theme }) =>
    isOutlined ? theme.colors.nonary : theme.colors.primary};
  color: ${({ isOutlined, theme }) =>
    isOutlined ? theme.colors.eleventh : '#FFF'};
  padding: 0 32px;
  ...
  cursor: pointer;
  border: ${({ isOutlined, theme }) =>
    isOutlined ? `1px solid ${theme.colors.primary}` : 0};
